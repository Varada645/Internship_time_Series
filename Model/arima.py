...
for disease in diseases:
    ...
    try:
        ...
        arima_forecast = fit.forecast(steps=forecast_steps)
        
        # Reverse transformations
        if d > 0:
            last_values = train_data.iloc[-d:].values
            arima_forecast = np.cumsum(np.concatenate([last_values, arima_forecast]))[d:]
        arima_forecast = np.expm1(arima_forecast)
        arima_forecast = np.maximum(arima_forecast, 0)
        
        # RMSE for test set
        arima_test_forecast = arima_forecast[:len(test)]
        arima_rmse = np.sqrt(mean_squared_error(test[disease].values, arima_test_forecast))
        
        # Additional metrics: MAE and MAPE
        arima_mae = mean_absolute_error(test[disease].values, arima_test_forecast)
        arima_mape = mean_absolute_percentage_error(test[disease].values, arima_test_forecast)
        ...
        # Save results
        with open(f"{disease_dir}/arima_results.txt", "w") as f:
            f.write(f"ARIMA RMSE: {arima_rmse:.2f}\n")
            f.write(f"ARIMA MAE: {arima_mae:.2f}\n")
            f.write(f"ARIMA MAPE: {arima_mape:.2f}%\n")
        ...
        forecast_df.to_csv(f"{disease_dir}/arima_forecast.csv", index=False)
        ...
        plt.title(f'{disease} ARIMA Forecast')
        plt.legend()
        plt.grid(True)
        plt.savefig(f"{disease_plot_dir}/arima_forecast_plot.png")
        plt.close()
        
        print(f"{disease} ARIMA RMSE: {arima_rmse:.2f}, d={d}")
    
    except Exception as e:
        print(f"Error for {disease}: {e}")

print("✅ ARIMA forecasts done.")
